...
#[test]
fn test_arithmetic_circuit_with_vector_commitment() {
  let generators = generators(128);
  let g = generators.g().point();
  let h = generators.h().point();
  ...
  fn gadget(
    ...
    let vc_var_2 = circuit
      .add_to_vector_commitment(vc, circuit.prover().then(|| circuit.unchecked_value(y_var)));
    let mut vc_constraint = Constraint::new("vc");
    vc_constraint
      .weight(product_l, <Ristretto as Ciphersuite>::F::ONE)
      .weight(product_r, <Ristretto as Ciphersuite>::F::ONE)
      .weight_variable_in_vector_commitment(vc, vc_var_1, -<Ristretto as Ciphersuite>::F::ONE)
      .weight_variable_in_vector_commitment(vc, vc_var_2, -<Ristretto as Ciphersuite>::F::ONE);
    circuit.constrain(vc_constraint);
  }

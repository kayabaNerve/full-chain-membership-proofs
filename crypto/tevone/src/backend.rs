  ...
  (
    ...
    impl Field for $FieldName {
      ...
      fn sqrt(&self) -> CtOption<Self> {
        const TM1D2: U256 = U256::from_be_hex($TM1D2);
        let mut words = [0u64; 4];
        let mut internal_word = 0;

        let words_ref = TM1D2.as_words();
        let thirty_two = (words_ref.len() == 8);
        assert!((words_ref.len() == 4) || thirty_two, "only 32 and 64-bit platforms are supported");

        // TODO: This is untested on 32-bit platforms
        for word in words_ref {
          words[internal_word / 2] |=
            u64::from(if (internal_word % 2) == 1 { word << 32 } else { *word });
          internal_word += if thirty_two { 1 } else { 2 };
        }
        ff::helpers::sqrt_tonelli_shanks(self, words)
      }
